# SITE-BRAND-001 — Stage 3B-R implementation map

Status: active implementation contract.
Before reference: live/current site at 2ac8e42ee467f6dd37cf4feef51102e4220ddd31.
Approved reference: docs/design/site-brand-001/approved/README.md and the hashed B/A renders listed in SHA256SUMS.txt.
...
- APPROVED TARGET: B folio route: editorial masthead, visible seven-direction problem map, one material lead story, ruled supporting reading list, an A evidence strip, tool dossier, service route rows, and a closing contact route.
...
- APPROVED TARGET: B reading desk: route folio, editorial library statement, seven-direction index, featured spread, and a ruled archive of articles with visible topic/date/type evidence.
...
- APPROVED TARGET: B single reading route: folio and route rail, material opening spread, explicit evidence/trace annotation, uninterrupted reading surface, and next-check navigation.
...
- APPROVED TARGET: B tool route: dossier masthead, editorial product record, material cover/evidence image, explicit status and fit, and a preparation/next-check rail.
...
- APPROVED TARGET: B service route: folio statement, three frozen services as annotated editorial rows, cause/check/action process, and a restrained contact route.
...
- APPROVED TARGET: B editorial profile: route folio, material workspace image, “who/why” statement, working method as cause/check/action notes, and a contact continuation.
...
- APPROVED TARGET: B closing route: dark/navy contact folio, direct question statement, ruled channel rows for contacts and project reading, and clear response expectation.
...
For each route, evidence must be captured as BEFORE (2ac8e42) → APPROVED REFERENCE (B/A render) → AFTER (new browser render) at 1440 and 390. A build, a changed color palette, copy, CTA, padding, or isolated CausalTrace block is not structural fidelity. Review Chair may not accept without artifact truth and reference fidelity.
