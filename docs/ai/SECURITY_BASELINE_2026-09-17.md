...
Implementation использует safe DOM APIs, task-scoped generated media, portable project-package ffmpeg/ffprobe resolver и current Brand SOT. Windows media binaries are dev-only; wrapper licenses are LGPL-2.1 and selected Windows binary metadata is GPLv3. `npm audit --omit=dev --json` returned 4 pre-existing/transitive vulnerabilities (3 high, 1 critical), so dependency security remains an explicit unresolved follow-up and is not treated as a clean release certification. PASS static baseline не является полной production security certification.
