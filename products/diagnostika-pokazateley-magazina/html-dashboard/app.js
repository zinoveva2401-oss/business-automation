...
const FIELD_DEFINITIONS = [
  ...
  { key: 'checks', label: 'Количество чеков', inputMode: 'numeric', positive: true },
  { key: 'items', label: 'Количество проданных товаров', inputMode: 'numeric', positive: true },
  { key: 'visitors', label: 'Количество посетителей', inputMode: 'numeric', positive: true },
...
function diagnosisContent(zone, changes, down) {
  ...
  if (zone === 'Конверсия') return {
    fact: down.traffic ? 'Конверсия снизилась сильнее трафика.' : 'Трафик сохранился или вырос, но покупок стало меньше.',
